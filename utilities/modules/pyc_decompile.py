import subprocess
from os import PathLike, makedirs, path, walk, pardir, sep, getcwd
...
def decompile_entry_points(entry_points):
    for pyc in entry_points:
        out_dir = path.abspath(
            path.join(path.abspath(path.join(pyc, pardir)), pardir))+sep+'Source_Code'
        if not path.exists(out_dir):
            makedirs(out_dir)
        decompile_pyc(pyc, out_dir+sep+path.basename(pyc)
                      [:-4]+'.py', log=True, _path=path.join(pardir, pardir))


def decompile_pyc(pyc_file: PathLike or str, output: PathLike or str, log=False, _path=getcwd()):
    process = subprocess.Popen(f"{_path}/utilities/bin/pycdc.exe {pyc_file}",
    ...
    if log:
        printSucc(
            f'Decompiled {path.basename(pyc_file)} --> {path.basename(output)}')
